...
(*                            equipping a type with a predType structure,     *)
(*                            which interoperates better with coercion        *)
(*                            subtyping. This is used, e.g., for finite sets, *)
(*                            so that finite groups inherit the membership    *)
(*                            operation by coercing to sets.                  *)
...
(*                            predicates (see below), as this will allow      *)
(*                            passing in directly collections that implement  *)
(*                            predType by coercion as described above,        *)
(*                            e.g.,  finite sets.                             *)
...
(*                   mem A == an applicative equivalent of collective         *)
(*                            predicate A: mem A x simplifies to x \in A,     *)
(*                            as mem A has in fact type mem_pred T.           *)
(* --> In user notation collective predicates _only_ occur as arguments to    *)
(*     mem: A only appears as (mem A). This is hidden by notation, e.g.,      *)
(*     x \in A := in_mem x (mem A) here, enum A := enum_mem (mem A) in        *)
(*     fintype. This makes it possible to unify the various ways in which A   *)
(*     can be interpreted as a predicate, for both pattern matching           *)
(*     and display.                                                           *)
...
(* eta-long MemPred (fun x => pA x) form, where pA is the pred interpretation *)
(* of A following its predType pT, i.e., the _expansion_ of topred A. For a   *)
(* pred T evar ?P, (mem ?P) converts MemPred (fun x => ?P x), whose argument  *)
(* is a Miller pattern and therefore always unify: unifying (mem A) with      *)
(* (mem ?P) always yields ?P = pA, because the rigid constant MemPred aligns  *)
(* the unification. Furthermore, we ensure pA is always either A              *)
(* or toP .... A where toP ... is the expansion of @topred T pT, and toP is   *)
(* declared as a Coercion, so pA will _display_ as A in either case, and the  *)
(* instances of @mem T (predPredType T) pA appearing in the premises          *)
(* or right-hand side of a generic lemma parameterized by ?P will be          *)
(* indistinguishable from @mem T pT A.                                        *)
(*   Users should take care not to inadvertently "strip" (mem A) down to the  *)
(* coerced A, since this will expose the internal toP coercion: Rocq could    *)
(* then display terms A x that cannot be typed as such. The topredE lemma can *)
(* be used to restore the x \in A syntax in this case. While -topredE can     *)
(* conversely be used to change x \in P into P x for an applicative P, it is  *)
(* safer to use the inE, unfold_in or and memE lemmas instead, as they do not *)
(* run the risk of exposing internal coercions. As a consequence it is better *)
(* to explicitly cast a generic applicative predicate to simpl_pred using the *)
(* SimplPred constructor when it is used as a collective predicate (see, e.g.,*)
...
(* {pred T}. This works better for types such as {set T} that have subtypes   *)
(* that coerce to them, since the same coercion will be inserted by the       *)
(* application of mem, or of any lemma that expects a generic collective      *)
(* predicates with type {pred T} := pred_sort (predPredType T) = pred T; thus *)
(* {pred T} should be the preferred type for generic collective predicate     *)
(* parameters.                                                                *)
...
(* e.g., ##|'I_n| for the cardinal of the (finite) type of integers           *)
(* less than n.                                                               *)
(******************************************************************************)
...
(* class, which crucially does _not_ coerce to Funclass. Term whose type P    *)
(* coerces to {pred T} cannot be applied to arguments, but they _can_ be used *)
(* as if P had a canonical predType instance, as the coercion will be inserted*)
(* if the unification P =~= pred_sort ?pT fails, changing the problem into the*)
(* trivial {pred T} =~= pred_sort ?pT (solution ?pT := predPredType P).       *)
...
(* canonical instance of predType for P would have solved the deferred        *)
(* constraint.                                                                *)
...
(* apply such definitions and lemmas directly to values of types that         *)
(* implement predType by coercion to {pred T} (values of types that implement *)
(* predType without coercing to {pred T} will have to be coerced explicitly   *)
(* using topred).                                                             *)
(******************************************************************************)
...
(* undesirable to have two distinct constants for what are essentially        *)
(* identical coercion functions, as this confuses the SSReflect keyed         *)
(* matching algorithm.                                                        *)
(* While the Rocq Coercion declarations appear to disallow such Coercion      *)
(* aliasing, it is possible to work around this limitation with a combination *)
(* of modules and functors, which we do below.                                *)
(*   In addition we also give a predType instance for simpl_pred, which will  *)
(* be preferred to the {pred T} coercion to solve                             *)
(* simpl_pred T =~= pred_sort ?pT constraints; note however that the          *)
(* pred_of_simpl coercion _will_ be used when a simpl_pred T is passed as a   *)
(* {pred T}, since the simplPredType T structure for simpl_pred T is _not_    *)
(* convertible to predPredType T.                                             *)
(******************************************************************************)
...
(*   This lets us use types of sort predArgType as a synonym for their        *)
(* universal predicate. We define this predicate as a simpl_pred T rather     *)
(* than a pred T or a {pred T} so that /= and inE reduce (T x) and x \in T    *)
...
(* will avoid a redundant mem in a collective, e.g., passing (mem A) to a     *)
(* lemma exception a generic collective predicate p : {pred T} and premise    *)
(* x \in P will display a subgoal x \in A rather than x \in mem A.            *)
(*   Conversely, pred_of_mem will _not_ if it is used id (mem A) is used      *)
(* applicatively or as a pred T; there the simpl_of_mem coercion defined      *)
(* below will be used, resulting in a subgoal that displays as mem A x by     *)
(* simplifies to x \in A.                                                     *)
...
(* matching the various predicate forms. While all four follow a common       *)
(* pattern of using a canonical projection to match a particular form of      *)
(* predicate (in pred T, simpl_pred, mem_pred and mem_pred, respectively),    *)
(* and display the matched predicate in the structure type, each is in fact   *)
(* used for a different, specific purpose:                                    *)
...
(* - manifest_mem_pred: similar to manifest_simpl_pred, the one instance of   *)
(*   this structure matches manifest mem_pred values of the form Mem [eta ?p].*)
(*   The purpose is different however: to match and display in ?p the actual  *)
(*   predicate appearing in an ... \in ... expression matched by the          *)
(*   left hand side of the in_applicative component of inE; then              *)
(* - applicative_mem_pred is a telescope refinement of manifest_mem_pred p    *)
(*   with a default constructor that checks that the predicate p is the value *)
(*   of a registered_applicative_pred; any unfolding occurring during this    *)
(*   check does _not_ affect the value of p passed to in_applicative, since   *)
(*   that has been fixed earlier by the manifest_mem_pred match. In particular*)
(*   the definition of a predicate using the applicative_pred_of_simpl idiom  *)
(*   above will not be expanded - this very case is the reason in_applicative *)
(*   uses a mem_pred telescope in its left hand side. The more straightforward*)
