...
Notation diagonalisable_in D A := (similar_in_to D A is_diag_mx).
Notation diagonalisable A := (diagonalisable_in unitmx A).
Notation codiagonalisable_in D As := (all_similar_to D As is_diag_mx).
Notation codiagonalisable As := (codiagonalisable_in unitmx As).

Notation similar_trig P A := (similar_to P A is_trig_mx).
Notation trigonalisable_in D A := (similar_in_to D A is_trig_mx).
Notation trigonalisable A := (trigonalisable_in unitmx A).
Notation cotrigonalisable_in D As := (all_similar_to D As is_trig_mx).
Notation cotrigonalisable As := (cotrigonalisable_in unitmx As).
...
Lemma similar_diag_sum (F : fieldType) (m n : nat) (p_ : 'I_n -> nat)
  ...
  similar_diag (col_block_mx V_) f = [forall i, similar_diag (V_ i) f].
Proof.
move=> Vd Vf rfV; have aVf : stablemx (col_block_mx V_) f.
  rewrite (eqmx_stable _ (eqmx_col_block _)) stablemx_sums//.
  by move=> i; rewrite (eqmx_stable _ (genmxE _)).
apply/similar_diagPex/'forall_similar_diagPex => /=
    [[D /(similarPp aVf) VMf] i|/(_ _)/sigW Dof].
  exists (\row_i D 0 (OrdSum.superindex i)); apply/eqP.
  apply: (canLR (mulmxKp _)); rewrite ?row_base_free//.
  apply/row_matrixP => j; rewrite row_mul.
  have /(congr1 (row (OrdSum.superindex j))) := VMf.
  rewrite [row (OrdSum.superindex _) _]row_mul row_col_block.
  rewrite ?OrdSum.superindexK/= !row_mul !row_diag_mx !mxE -!scalemxAl -!rowE.
  rewrite row_col_block ?OrdSum.superindexK//.
  by case: _ / esym => /=; rewrite cast_ord_id.
exists (row_block_mx (fun i : 'I_n => projT1 (Dof i))); apply/similarW.
...
case: Dof => /= k /(similarPp); rewrite Vf => /(_ isT) ->.
...
Definition codiagonalisablePfull n (As : seq 'M[F]_n) :
  codiagonalisable As <-> exists m,
...
Lemma codiagonalisable_on (m n : nat) (V_ : 'I_n -> 'M[F]_m) (As : seq 'M[F]_m) :
  ...
  (forall i, codiagonalisable (map (restrictmx (V_ i)) As)) -> codiagonalisable As.
Proof.
...
Lemma diagonalisable_diag {n} (d : 'rV[F]_n) : diagonalisable (diag_mx d).
Proof.
...
Lemma diagonalisable_conj_diag m n (V : 'M[F]_(m, n)) (d : 'rV[F]_n) :
  stablemx V (diag_mx d) -> row_free V -> diagonalisable (conjmx V (diag_mx d)).
Proof.
...
Lemma codiagonalisableP n (fs : seq 'M[F]_n) :
  {in fs &, forall f g, comm_mx f g} /\ (forall f, f \in fs -> diagonalisable f)
  <-> codiagonalisable fs.
Proof.
...
apply/(@codiagonalisable_on _ _ _ (_ :: _) rs1) => [|i|i /=].
- apply: mxdirect_sum_eigenspace => i j _ _ rsij; apply/val_inj.
  by apply: uniqP rsij; rewrite ?inE.
